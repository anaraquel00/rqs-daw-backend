...
function originalNameFromRequest(s3Key, uploadedFile) {
    if (s3Key) {
        return path.basename(s3Key).replace(/^\d+_/, '').replace(/\.[^/.]+$/, '');
    }
    if (uploadedFile) {
...
router.post('/process', upload.single('audio'), async (req, res) => {
    ...
    try {
        ...
        if (result.code !== 0) {
            ...
            const validationDetail = status === 400 ? extractCliError(result) : undefined;
            if (status === 500) {
                console.error('[MASTERING V2] processing CLI error:', result.stderr);
            }
            return res.status(status).json({
                error: status === 400 ? 'Invalid Mastering V2 request.' : 'Mastering V2 processing failed.',
                ...(validationDetail ? { details: validationDetail } : {})
            });
        }
        ...
        if (!res.headersSent) {
            return res.status(error.statusCode || 500).json({
                error: error.statusCode === 400 ? error.message : 'Internal Mastering V2 error.'
            });
        }
    } finally {
