...
def apply_high_compression(
    ...
    """
    Apply the configured legacy high-band compressor with explicit wet/dry.

    amount=0.0 is an exact dry bypass.
    amount=1.0 preserves the configured legacy compressor output.
    Intermediate values interpolate between dry and compressed audio.
    """
    amount = float(amount)

    if not np.isfinite(amount) or not 0.0 <= amount <= 1.0:
        raise ValueError(
            "High compression amount must be finite and between 0.0 and 1.0."
        )
...
def resolve_legacy_faction(
    perfil: str,
    legacy_faction_override: str | None = None,
) -> str:
    # Legacy callers keep historical profile-based inference when no override
    # is supplied. V2 can provide the compatibility faction explicitly so the
    # new Atmosphere model is not coupled to this legacy inference.
    if legacy_faction_override is None:
        return "blue" if perfil in ["clear_sky", "clear sky", "aurora"] else "red"

    faction = str(legacy_faction_override).lower().strip()
    if faction not in ("blue", "red"):
        raise ValueError("Legacy faction override must be 'blue' or 'red'.")

    return faction


def masterize(input_path: str, output_path: str, estilo: str, intensidade: str, is_preview: bool = False, target_lufs_override: float | None = None, limiter_ceiling_override: float | None = None, side_saturation_amount: float = 1.0, transient_amount: float = 1.0, transient_max_boost_override: float | None = None, side_lowpass_cutoff_override: float | None = None, high_cleanup_amount: float = 1.0, high_compression_amount: float = 1.0, side_highpass_cutoff_override: float | None = None, mid_compression_enabled: bool = True, side_compression_enabled: bool = True, legacy_faction_override: str | None = None):
    ...
    try:
        ...
        faccao = resolve_legacy_faction(perfil, legacy_faction_override)
        ...
        if perfil == "thunder":
            ...
            comp_mid = Compressor(threshold_db=rms_mid_db + comp_threshold_modifier, ratio=max(1.0, final_ratio), attack_ms=25.0, release_ms=mid_release) if mid_compression_enabled else None
            ...
            comp_mid = Compressor(threshold_db=rms_mid_db + comp_threshold_modifier, ratio=max(1.0, final_ratio * 0.8), attack_ms=30.0, release_ms=mid_release) if mid_compression_enabled else None
            comp_high = Compressor(threshold_db=rms_high_db - 3.0 + comp_threshold_modifier, ratio=high_ratio, attack_ms=1.0, release_ms=high_release)
            ...
            comp_mid = Compressor(threshold_db=rms_mid_db - 1.0 + comp_threshold_modifier, ratio=max(1.0, final_ratio * 1.1), attack_ms=15.0, release_ms=mid_release) if mid_compression_enabled else None
            comp_high = Compressor(threshold_db=rms_high_db - 3.0 + comp_threshold_modifier, ratio=high_ratio, attack_ms=1.0, release_ms=high_release)
            ...
            comp_mid = Compressor(threshold_db=rms_mid_db + comp_threshold_modifier, ratio=max(1.0, final_ratio * 0.8), attack_ms=25.0, release_ms=mid_release) if mid_compression_enabled else None
            comp_high = Compressor(threshold_db=rms_high_db - 3.0 + comp_threshold_modifier, ratio=high_ratio, attack_ms=1.0, release_ms=high_release)

        mid_low_processed = comp_low(mid_low[np.newaxis, :], sample_rate)[0]
        if mid_compression_enabled:
            mid_mid_processed = comp_mid(mid_mid[np.newaxis, :], sample_rate)[0]
        else:
            mid_mid_processed = mid_mid
        ...
        mid_high_processed = apply_high_compression(
            mid_high_filtered,
            sample_rate,
            comp_high,
            high_compression_amount,
        )
        ...
        # Legacy keeps the historical 150 Hz Side HPF.
        # V2 can supply an explicit cutoff so the low-frequency stereo policy
        # is not hard-wired to the legacy Clear Sky voicing.
        if side_highpass_cutoff_override is None:
            side_highpass_cutoff_hz = 150.0
        else:
            side_highpass_cutoff_hz = float(side_highpass_cutoff_override)
            if not np.isfinite(side_highpass_cutoff_hz) or side_highpass_cutoff_hz <= 0.0:
                raise ValueError(
                    "Side high-pass cutoff override must be finite and greater than 0 Hz."
                )

        board_side = Pedalboard([
            HighpassFilter(cutoff_frequency_hz=side_highpass_cutoff_hz),
