...
$ValidationRoot = 'D:\RQS-Dev\mastering_v2_real_audio_behavioral_gate'
...
$CanonicalHarness = Join-Path $RunDir 'canonical_render.py'
...
Write-Host ''
Write-Host '============================================================'
Write-Host ' RQS MASTERING V2 — LOCAL REAL-AUDIO BEHAVIORAL PARITY'
Write-Host '============================================================'
...
$StandaloneHead = (& $Git.Source -C $StandaloneRepo rev-parse HEAD).Trim()
...
if ([double]::IsNaN($PreviewStart)) {
    throw 'PreviewStart is required for an explicit input.'
}
...
$ApiHeaders = @{ 'User-Agent' = 'rqs-mastering-v2-local-real-audio-behavioral-validator/1.0' }
...
Write-Host "MASTERING_V2_CANDIDATE_SHA256: $($v2Candidate.Hash)"
Write-Host "MASTERING_V2_CANONICAL_SHA256: $($v2Canonical.Hash)"
if ($v2Candidate.Hash -eq $v2Canonical.Hash) {
    Write-Host 'MASTERING_V2_WHOLE_FILE_HASH: SAME'
} else {
    Write-Host 'MASTERING_V2_WHOLE_FILE_HASH: DIFFERENT_BY_DESIGN / BEHAVIORAL_PARITY_REQUIRED'
}
...
standalone_repo = Path(sys.argv[1]).resolve()
mode = sys.argv[2]
source = Path(sys.argv[3]).resolve()
output = Path(sys.argv[4]).resolve()
preview_start = float(sys.argv[5]) if len(sys.argv) > 5 else 0.0

sys.path.insert(0, str(standalone_repo))
from src.controllers.mastering_v2 import masterize_v2
...
if mode == 'full':
    masterize_v2(str(source), str(output), is_preview=False, **common)
    raise SystemExit(0)
...
preview_frames = int(15.0 * sr)
if not math.isfinite(preview_start) or preview_start < 0:
    raise ValueError('preview_start must be finite and >= 0')

if total_frames > preview_frames:
    max_start = total_frames - preview_frames
    requested = int(round(preview_start * sr))
...
fd, name = tempfile.mkstemp(prefix='rqs_behavioral_preview_source_', suffix='.wav')
os.close(fd)
segment = Path(name)
try:
    sf.write(str(segment), x, int(read_sr), format='WAV', subtype='FLOAT')
    # The canonical release has centered-preview selection only. Feeding the
    # exact selected 15 s source segment into its preview path preserves the
    # canonical preview DSP behavior while independently validating the newer
    # preview_start_seconds transport/selection logic in the integration layer.
    masterize_v2(str(segment), str(output), is_preview=True, **common)
finally:
    segment.unlink(missing_ok=True)
'@ | Set-Content -LiteralPath $CanonicalHarness -Encoding UTF8
...
try {
    ...
    & $Python $CanonicalHarness $StandaloneRepo 'preview' $InputFile $PreviewCanonical ([string]$PreviewStart)
    if ($LASTEXITCODE -ne 0 -or -not (Test-Path $PreviewCanonical)) {
        throw 'Canonical behavioral Preview render failed.'
    }
    ...
    & $Python $CanonicalHarness $StandaloneRepo 'full' $InputFile $FullCanonical '0'
    if ($LASTEXITCODE -ne 0 -or -not (Test-Path $FullCanonical)) {
        throw 'Canonical Full Master render failed.'
    }
...
'@ | Set-Content -LiteralPath $CompareScript -Encoding UTF8
    ...
    if ($CompareExit -ne 0) {
        ...
        throw 'LOCAL_REAL_AUDIO_BEHAVIORAL_PARITY: FAIL'
    }
    $Compare = $CompareText | ConvertFrom-Json
    if ($Compare.status -ne 'PASS') { throw 'LOCAL_REAL_AUDIO_BEHAVIORAL_PARITY: FAIL' }

    Write-Host 'LOCAL_REAL_AUDIO_PREVIEW_15S_EXACT: PASS'
    Write-Host 'LOCAL_REAL_AUDIO_PREVIEW_SELECTED_WINDOW_BEHAVIORAL_PARITY: PASS'
    Write-Host 'LOCAL_REAL_AUDIO_FULL_CANONICAL_PARITY: PASS'
    Write-Host 'LOCAL_REAL_AUDIO_FULL_DURATION_PRESERVED: PASS'

    $Manifest = @(
        'MASTERING_V2_LOCAL_REAL_AUDIO_BEHAVIORAL_PARITY: PASS',
        ...
        "MASTERING_V2_CANDIDATE_SHA256: $($v2Candidate.Hash)",
        "MASTERING_V2_CANONICAL_SHA256: $($v2Canonical.Hash)",
        'MASTERING_V2_WHOLE_FILE_HASH: DIFFERENCE_ALLOWED_ONLY_WITH_BEHAVIORAL_PARITY',
    ...
    Write-Host 'MASTERING_V2_LOCAL_REAL_AUDIO_BEHAVIORAL_PARITY: PASS'
