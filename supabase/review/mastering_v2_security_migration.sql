...
--
-- This preflight intentionally validates the minimum schema/security
-- contract required by Mastering V2. It supports both the real production
-- baseline (broad legacy browser grants + own-profile UPDATE policy) and the
-- isolated staging baseline after the reviewed staging-alignment script.
-- Unexpected UPDATE policies fail closed.
-- =========================================================
...
begin
  ...
  if exists (
    select 1 from public.profiles
    where completed_masters is null or completed_masters < 0
  ) then
  ...
  if v_update_policy_count > 1 then
    raise exception 'UNEXPECTED_MULTIPLE_PROFILE_UPDATE_POLICIES';
  end if;

  if v_update_policy_count = 1 and not exists (
    select 1 from pg_policies
    where schemaname = 'public'
      and tablename = 'profiles'
      and cmd = 'UPDATE'
      and policyname = 'Permitir que usuários atualizem seus próprios perfis'
      and roles = array['authenticated']::name[]
      and replace(coalesce(qual, ''), ' ', '') in ('(auth.uid()=id)', '((auth.uid())=id)')
      and replace(coalesce(with_check, ''), ' ', '') in ('(auth.uid()=id)', '((auth.uid())=id)')
  ) then
    raise exception 'UNEXPECTED_PROFILE_UPDATE_POLICY';
  end if;
...
-- =========================================================
-- 1. RETIRE ALL DIRECT BROWSER WRITES TO PROFILES
--
-- Production evidence showed the legacy table ACL granted broad DML to
-- browser roles and RLS allowed authenticated users to UPDATE their own row.
-- That made role/completed_masters client-mutable. Project 1 moves mastering
-- authority to the backend service role, so browser roles become read-only.
-- =========================================================
...
-- Preserve authenticated owner reads. RLS remains the row boundary.
grant select
on table public.profiles
to authenticated;

-- The legacy own-profile UPDATE policy becomes unnecessary/inert once direct
-- browser writes are retired. Remove the known policy to make the contract
...
-- Free = three Full Masters total for Final Beta.
-- Premium does not consume the Free quota.
-- Preview never calls these RPCs.
-- Old abandoned reservations are released after two hours.
-- =========================================================
...
begin
  ...
  select p.role::text, p.completed_masters::bigint
  ...
  set status = 'released', finalized_at = v_now
  where user_id = p_user_id
    and status = 'reserved'
    and created_at < v_now - interval '2 hours';

  select count(*)::bigint into v_reserved
  ...
  if v_counts_quota and (v_completed + v_reserved) >= 3 then
    raise exception using errcode = 'P0001', message = 'MASTERING_QUOTA_EXCEEDED';
  end if;

  insert into public.mastering_quota_reservations (
    id, user_id, counts_quota, status, created_at
  ) values (
    p_reservation_id, p_user_id, v_counts_quota, 'reserved', v_now
  );
...
begin
  ...
  -- Same lock order as reserve: profile first, reservation second.
  perform 1
  from public.profiles as p
  where p.id = p_user_id
  for update;
  ...
  select r.status, r.counts_quota
    into v_status, v_counts_quota
  ...
  update public.mastering_quota_reservations
  set status = 'completed', finalized_at = clock_timestamp()
...
begin
  ...
  update public.mastering_quota_reservations
  set status = 'released', finalized_at = clock_timestamp()
  where id = p_reservation_id and user_id = p_user_id;
...
grant execute on function public.reserve_mastering_quota(uuid, uuid) to service_role;
grant execute on function public.confirm_mastering_quota(uuid, uuid) to service_role;
grant execute on function public.release_mastering_quota(uuid, uuid) to service_role;
